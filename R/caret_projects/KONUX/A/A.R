...
df = read.csv("df9.csv", header = FALSE)

nrow(df)
str(df)
summary(df)

lmfit = lm(df$V33~., data=df)
summary(lmfit)
plot(lmfit)

glmfit= glm(df$V33~., data=df, family=poisson())
summary(glmfit)
plot(glmfit)

# fit model
?gbm.fit
fit = gbm.fit(df$V33~., df$V33,distribution="gaussian")
# summarize the fit
summary(fit)
# make predictions
predictions <- predict(fit, df)
# summarize accuracy
rmse <- mean((df$V33 - predictions)^2)
print(rmse)
